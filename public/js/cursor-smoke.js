(function () {
  ...
  /* ── tunable config (mirrors common WebGL fluid-sim knobs) ── */
  const config = {
    SIM_RESOLUTION: 128,
    DYE_RESOLUTION: 1024,
  ...
  try {
    ...
    if (isWebGL2) {
      ...
      var supportLinearFloat = gl.getExtension('OES_texture_float_linear');
    } else {
  ...
  const pointer = { x: 0, y: 0, dx: 0, dy: 0, moved: false, color: [0.15, 0.45, 0.9] };
  ...
  window.addEventListener('mouseleave', () => { lastX = null; lastY = null; }, { passive: true });
  window.addEventListener('resize', () => { if (resizeCanvas()) initFramebuffers(); }, { passive: true });
  ...
  requestAnimationFrame(step);
  window.__smokeDebug = { config, splat, step, gl, dye: () => dye, velocity: () => velocity, splatProgram, displayProgram };
  } catch (e) {
    window.__smokeError = e.message;
    canvas.remove();
  }
})();
